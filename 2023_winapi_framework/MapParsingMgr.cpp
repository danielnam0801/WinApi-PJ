...
std::unique_ptr<tson::Map> MapParsingMgr::parseMap(const std::string& filename, std::unique_ptr<tson::IDecompressor<std::vector<uint8_t>, std::vector<uint8_t>>> decompressor)
{
    tson::Tileson t;
...
bool MapParsingMgr::parseProject(const std::string& filename)
{
...
void MapParsingMgr::drawObjectLayer(tson::Layer& layer)
{
...
void MapParsingMgr::drawLayer(tson::Layer& layer)
{
 ...
 * @return
 */
fs::path MapParsingMgr::getTilesetImagePath(const tson::Tileset& tileset)
{
...
#if __clang__
fs::path MapParsingMgr::getMacApplicationFolder(bool isAppPath)
{
    char buf[PATH_MAX];
